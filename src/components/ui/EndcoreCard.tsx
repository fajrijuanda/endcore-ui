...
export const EndcoreCard = ({ title, status = 'neutral', children, className, action }: EndcoreCardProps) => {
    ...
    return (
        <div className={cn(
            "group relative bg-white dark:bg-[#1A1A1A] border border-zinc-200 dark:border-white/10 rounded-[2.5rem] overflow-hidden transition-all duration-500 hover:-translate-y-1.5 hover:shadow-2xl flex flex-col",
            className
        )}>
